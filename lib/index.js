  ...
  function liveClassScores(triggers, classId) {
    ...
    classAssignments.on("ready", function () {
      ...
      var mySelect = triggers.select("\n        SELECT\n          students.name  AS student_name,\n          students.id    AS student_id,\n          assignments.id AS assignment_id,\n          assignments.name,\n          assignments.value,\n          scores.score\n        FROM\n          scores\n        INNER JOIN assignments ON\n          (assignments.id = scores.assignment_id)\n        INNER JOIN students ON\n          (students.id = scores.student_id)\n        WHERE\n          assignments.class_id = " + classId + "\n      ", {
        assignments: function (class_id, id) {
          return class_id === classId ? { assignment_id: id } : false;
        },
        students: function (id) {
          return studentIds.indexOf(id) !== -1 ? { student_id: id } : false;
        },
        scores: function (assignment_id, student_id) {
          return assignmentIds.indexOf(assignment_id) !== -1 ? { assignment_id: assignment_id, student_id: student_id } : false;
        }
      });
