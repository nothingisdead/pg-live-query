	...
	_createClass(LiveSQL, {
		...
		_updateQuery: {
			value: function _updateQuery(queryHash) {
				...
				var pgHandle, queryBuffer, diff, rows, _iteratorNormalCompletion, _didIteratorError, _iteratorError, _iterator, _step, updateHandler;
				...
				return _regeneratorRuntime.async(function _updateQuery$(context$2$0) {
					while (1) switch (context$2$0.prev = context$2$0.next) {
						...
						case 6:
							...
							rows = queryBuffer.data.map(function (hash, index) {
								return common.getRow(hash);
							});
							_iteratorNormalCompletion = true;
							_didIteratorError = false;
							_iteratorError = undefined;
							...
							for (_iterator = _core.$for.getIterator(queryBuffer.handlers); !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
								...
								updateHandler(filterHashProperties(diff), rows);
							}
							context$2$0.next = 22;
							break;
