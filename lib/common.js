...
// 	_.forOwn(pg.pools.all, (pool) => {
// 		let log = {
// 			name      : pool.getName(),
// 			size      : pool.getPoolSize(),
// 			avaliable : pool.availableObjectsCount(),
...
module.exports = exports = {
	...
	getClient: function getClient(connectionString) {
		return new _core.Promise(function (resolve, reject) {
			pg.connect(connectionString, function (error, client, done) {
				clients++;
				if (error) reject(error);else resolve({ client: client, done: (function (_done) {
						var _doneWrapper = function done() {
							return _done.apply(this, arguments);
						};

						_doneWrapper.toString = function () {
  ...
  */
	applyDiff: function applyDiff(data, diff) {
		...
		diff.moved !== null && diff.moved.forEach(function (moved) {
			newResults[moved.new_index - 1] = data[moved.old_index - 1];
		});

		diff.added !== null && diff.added.forEach(function (added) {
			if (!refs[added._hash]) {
				rows[added._hash] = _core.Object.freeze(_.omit(added, ["_hash", "_index"]));
			}
		...
		return newResults.filter(function (hash) {
			return hash !== undefined;
		});
	},

	getRow: function getRow(hash) {
		return rows[hash];
	}
};
