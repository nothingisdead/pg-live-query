	...
	_prototypeProperties(LiveSelect, null, {
						...
						if (_.isEmpty(fetch)) {
							...
							client.query(sql, _this.params, function (error, result) {
								...
								result.rows.forEach(function (row) {
									return _this.rowCache.set(row._hash, row);
								});
								done();
								_this.update(changes);
							});
			...
			value: function update(changes) {
				...
				var changes = changes.map(function (change) {
					...
					if (args[2] === null) {
						...
						return _this.emit("error", new Error("CACHE_MISS (" + args[0] + "): " + key));
					}
		...
		stop: {
			value: function stop() {
				...
				this.parent.removeListener("change:" + this.queryHash, this.throttledRefresh);
			},
			writable: true,
			configurable: true
		}
...
function deinit(client, callback) {
	...
	var sql = ["DROP VIEW " + viewName, "DROP VIEW " + hashViewName, ["DELETE FROM _liveselect_queries\n\t\t\tWHERE id = $1", [staticHash]], ["DELETE FROM _liveselect_column_usage\n\t\t\tWHERE query_id = $1", [staticHash]]];
